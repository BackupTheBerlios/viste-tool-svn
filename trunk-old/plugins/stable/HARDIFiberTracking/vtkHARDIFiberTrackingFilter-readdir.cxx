...
void vtkHARDIFiberTrackingFilter::ExecuteMain()
{
	cout << "vtkHARDIFiberTrackingFilter::Execute()" << endl;
...
*/
	...
	vtkIntArray *trianglesArray =  vtkIntArray::New();
		...
		vtkPointData * imagePD = this->HARDIimageData->GetPointData();
	if ((imagePD->GetArray("Spherical Directions")))
			{
			cout << "  spherical directions ie angles " << endl;
			 
		}
			if ( (imagePD->GetArray("Triangles")) )
		{
			cout << " Triangles exist!! " << endl;

		}

	cout << "Compute geometry from directions \n";
		//triangulatefromunitvectors !!!
	// compute unit vectors and angles array and create triangulation (an array called triangles)
	//get unit vectors, angles and triangles
	/* Alternative for Remco */
	...
	/* If the data already has directions */ 
	//vtkPointData * imagePD = this->HARDIimageData->GetPointData();
		//imagePD->GetArray("Spherical Directions")
	...
	for(int i=0; i< directionPoints->GetNumberOfPoints();i++)
	{
		...
		cout << i << " " << directionPoints->GetPoint(i)[0] << " " << directionPoints->GetPoint(i)[1] << " " << directionPoints->GetPoint(i)[2] << " " << endl;
	...
	triangulator->triangulateFromUnitVectors(directionPoints, trianglesArray);
	delete triangulator;
	...
	cout << numberOfTessPoints << "points "<< endl;
	...
	for (int i = 0; i < numberOfTessPoints; ++i)
	{
		...
		anglesArray.push_back(sc);
	}
	 
	cout << "Compute geometry from directions. End. \n";

 
	 
	//set the unit vectors
	tracker->setUnitVectors(unitVectors);
	cout << "unit vectors ok" << endl;
	//set the current time
	//unsigned int startTime = clock();
	...
	for (unsigned int iterations = 0; iterations < NUMBEROFITERATIONS; ++iterations)
	{
		...
		for (int ptId = 0; ptId < seedPoints->GetNumberOfPoints(); ptId++)
		{
			...
			// Calculate the fiber in positive and negative direction
			tracker->calculateFiberSH( 1, &streamlinePointListPos, anglesArray, trianglesArray, NUMBEROFITERATIONS, CLEANMAXIMA, TRESHOLD);
			tracker->calculateFiberSH(-1, &streamlinePointListNeg, anglesArray, trianglesArray, NUMBEROFITERATIONS, CLEANMAXIMA, TRESHOLD);
