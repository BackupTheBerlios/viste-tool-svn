...
vtkFiberMapper::vtkFiberMapper()
{
  ...
  this->ShaderProgram->SetAttrib(1, "Tangent");
  this->ShaderProgramShadow = vtkAnisoLiShadowMapSP::New();
  this->ShaderProgramShadow->SetAttrib(1, "Tangent");
  this->ShaderProgramShadow->SetAttrib(2, "LineID");
...
void vtkFiberMapper::Render(vtkRenderer* aren, vtkActor* act)
{
  ...
  if ( !this->Initialized )
    {
		this->Initialize(aren->GetRenderWindow());
    }
  ...
  //if (!this->Colors) glColor4f(1.0, 1.0, 1.0, 1.0);
  if (!this->Colors)
    { // use the color of the actor
    double rgb[3];
    act->GetProperty()->GetColor(rgb);
    glColor3dv(rgb);
    }
  ...
  if (this->Lighting) // && this->ShaderProgram)
    ...
    vtkMyShaderProgram::Deactivate();
    glDisable(GL_LIGHTING);
    }

  // set matrix mode for actors 
  //glMatrixMode(GL_MODELVIEW); // put in PostShadowMapRender() now.
...
void vtkFiberMapper::Initialize(vtkRenderWindow * renwin)
{
  this->Initialized = false;

  vtkOpenGLExtensionManager* extensions = vtkOpenGLExtensionManager::New();
  extensions->SetRenderWindow(renwin); // set render window to current render window
...
bool vtkFiberMapper::IsRenderSupported()
{
  if (!this->Initialized)
    {
	// Evert: using NULL here as the RenderWindow pointer may cause crashes 
	// on some systems. However, this function does not appear to be called at
