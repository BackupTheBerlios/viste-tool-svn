                                     ...
                                     int wholeExtent[6])
{
  ...
  if( !  nifti_validfilename(ImageFileName.c_str())  ) vtkErrorMacro("bad fname input") ;
  ...
  if(foundNiftiHeader){
    ...
    m_NiftiImage =  nifti_convert_nhdr2nim(niftiHeader, ImageFileName.c_str());
    } else if(foundAnalayzeHeader){
  ...
  m_NiftiImage->fname =  nifti_makehdrname(ImageFileName.c_str(),  m_NiftiImage->nifti_type, check, comp);
  m_NiftiImage->iname =  nifti_makeimgname(ImageFileName.c_str(),  m_NiftiImage->nifti_type, check, comp);
  printf( "nifti_set_iname_offset  " ) ;
  ...
  if(foundAnalayzeHeader){
    ...
    mat44 tempMat44 =  nifti_make_orthog_mat44( q[0][0], q[0][1], q[0][2],
        q[1][0], q[1][1], q[1][2],
        q[2][0], q[2][1], q[2][2] );
    ...
    tempMat44.m[2][3] = m_NiftiImage->dim[3];
     ...
     nifti_mat44_to_quatern( tempMat44,
  ...
  nhdr =  nifti_convert_nim2nhdr(m_NiftiImage);    // create the nifti1_header struct
  ...
  if( m_NiftiImage->nifti_type != NIFTI_FTYPE_NIFTI1_1 ){
    ...
    if( m_NiftiImage->iname == NULL ){ // then make a new one
      m_NiftiImage->iname =  nifti_makeimgname(m_NiftiImage->fname,m_NiftiImage->nifti_type,0,0);
  ...
  if( ! znz_isnull(imgfile) && m_NiftiImage->nifti_type == NIFTI_FTYPE_NIFTI1_1 ){
	    printf( "nifti_set_iname_offset2 %d ",NIFTI_FTYPE_NIFTI1_1 ) ;
    fp = imgfile;
    }
  else {
    fp =  znzopen( m_NiftiImage->fname , opts ,  nifti_is_gzfile(m_NiftiImage->fname) ) ;
    if( znz_isnull(fp) ){
  ...
  if( m_NiftiImage->nifti_type != NIFTI_FTYPE_NIFTI1_1 ){ // get a new file pointer
    ...
    if( ! znz_isnull(imgfile) ){
      ...
      fp =  znzopen( m_NiftiImage->iname , opts ,  nifti_is_gzfile(m_NiftiImage->iname) ) ;
      if( znz_isnull(fp) ) vtkErrorMacro("cannot open image file") ;
      }
    }

   znzseek(fp, m_NiftiImage->iname_offset, SEEK_SET);  // in any case, seek to offset
                               ...
                               int extent[6], int wholeExtent[])
{
  ...
  if( ! znz_isnull(imgfile) ){
    ...
    fp =  znzopen( ImageFileName.c_str() , opts ,  nifti_is_gzfile(ImageFileName.c_str()) ) ;
    if( znz_isnull(fp) ) vtkErrorMacro("cannot open image file") ;
    }
  numberOfBytes = this->getImageSizeInBytes();

   znzrewind(fp);
   znzseek(fp, iname_offset, SEEK_SET);  // in any case, seek to offset
  if( write_data ) {
     nifti_write_buffer(fp, out_p, numberOfBytes);
    }
  if( ! leave_open )  znzclose(fp);
