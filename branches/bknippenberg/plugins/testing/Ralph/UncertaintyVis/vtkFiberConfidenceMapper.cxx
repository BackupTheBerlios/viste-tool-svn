...
void vtkFiberConfidenceMapper::RenderStreamlines( float min, float max )
{
	...
	int nrLines = this->GetTable()->GetNumberOfValues();
	...
	{
		vtkIdType nrPtIds, * ptIds = 0, id = this->GetTable()->GetIdAt( i ); // int nrPtIds, * ptIds = 0;
		lines->GetCell( id, nrPtIds, ptIds ); //lines->GetCell( this->GetTable()->GetIdAt( i ), nrPtIds, ptIds );
		cellIndex += (nrPtIds + 1);
