...
QString bmiaNiftiReader::readNIfTIFile(const char * filename, bool showProgress)
{
	...
	if (showProgress)
	{
		...
		progress->setLabelText("Reading NIfTI file...");
		progress->setWindowTitle("NIfTI Reader");
	...
	if (progress)
	{
		...
		progress->setLabelText("Parsing NIfTI data...");
	}
...
bool bmiaNiftiReader::determineDataType()
{
	...
	if (ext)
	{
		...
		if (this->compareMiNDID(ext->edata, "DISCSPHFUNC", ext->esize - 8))
		{
		...
		// Spherical harmonics
		if (this->compareMiNDID(ext->edata, "REALSPHARMCOEFFS", ext->esize - 8))
		{
		...
		// DTI tensors
		if (this->compareMiNDID(ext->edata, "DTENSOR", ext->esize - 8))
		{
...
vtkImageData * bmiaNiftiReader::parseDTIVolume()
{
	...
	if (ext)
	{
		...
		if (this->compareMiNDID(ext->edata, "DTENSOR", ext->esize - 8) && (extPos + 6) < this->NiftiImage->num_ext)
		{
