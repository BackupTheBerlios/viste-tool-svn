...
void main()
{
  ...
  vec3 EllipsoidRadii = vEigenvalues * vec3(GlyphScaling);
...
*/
	vEigenvalues[2] = 0.1;
	if (vEigenvalues[1] < 0.1) vEigenvalues[1] = 0.1;
	EllipsoidRadii = vEigenvalues * vec3(GlyphScaling);
	dist = RayAlignedEllipsoidIntersection(tEyePos, tEyeDir, EllipsoidRadii);
	}
...
//  color.rgb = vec3(cl);
  ...
  normal = transpose(rot)*normalize(normal);
