...
//namespace bmia {
	...
	{
		double range  = this->Max; //(this->Max - this->Min);
		assert( range != 0.0 );		
		...
		for( ; iter != this->Table->end(); iter++ )
		{
			...
			double normDist = iter->first / range;
			iter->first = normDist;
		}
	...
	////////////////////////////////////////////////////////////////////////
	void vtkDistanceTable::Write( const std::string & fileName )
	{
		std::ofstream outFile;
		outFile.open( fileName.c_str() );
		outFile << this->GetNumberOfElements() << std::endl;
		std::vector< std::pair< double, int > >::iterator iter = this->Table->begin();
		for( ; iter != this->Table->end(); ++iter )
			outFile << (*iter).first << " " << (*iter).second << std::endl;
		outFile << this->Min << std::endl;
		outFile << this->Max << std::endl;
		outFile.close();
	}

	////////////////////////////////////////////////////////////////////////
	void vtkDistanceTable::Read( const std::string & fileName )
	{
		std::ifstream inFile;
		...
		for( int i = 0; i < nrElems; ++i )
		{
			double distance;
			int index;
			inFile >> distance >> index;
			this->Add( distance, index );
		}
	}
//}
